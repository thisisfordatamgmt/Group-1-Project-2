...
The question itself is non-trivial because it requires you to use the provided CPI to first calculate the inflation rate by utilizing the ```LAG()``` function, and then chart the results over time to see how, and to what extent, each event impacted the inflation rate. 
